...
def build_cnn(deep, CL, HL, sz, input_var=None, BN=False, channels=1):
    ...
    for t in range(deep):
        for i in range(CL):
            network = lasagne.layers.Conv2DLayer(
                network, num_filters=flt, filter_size=(3, 3),
                nonlinearity=lasagne.nonlinearities.rectify,
...
def run_method(method, dataset='MNIST', model='mlp', deep=5, CL=3, HL=1, BN=False, num_epochs=50, alpha=0.1, mu=0.9,
    ...
    if model == 'mlp':
        ...
        network = build_cnn(deep, CL, HL, sz, input_var, BN, channels)
    else:
